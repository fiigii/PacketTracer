...
internal class Packet256Tracer
{
    ...
    private static readonly Vector256<float> ZeroToSeven = Vector256.Create(0f, 1f, 2f, 3f, 4f, 5f, 6f, 7f);
    ...
    internal unsafe void RenderVectorized(Scene scene, int* rgb)
    {
        ...
        for (int y = 0; y < Height; y++)
        {
            ...
            {
                float fx = x;
                Vector256<float> Xs = Add(Vector256.Create(fx), ZeroToSeven);
                var dirs = GetPoints(Xs, Vector256.Create((float)y), camera);
    ...
    {
        var widthVector = Vector256.Create((float)Width);
        var heightVector = Vector256.Create((float)Height);
